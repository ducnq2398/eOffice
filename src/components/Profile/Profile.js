...
function Profile() {
  ...
  function handleChangePassword(e){
    ...
    var pattern = new RegExp(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{6,}$/);
    if(password.current_pass.trim() === ''){
      ...
      userListAPI.changePassword(params).then(function(){
        removeUserSession();
        history.push('/');
  ...
  function handleNewAvatar(e){
    ...
    userListAPI.changeAvatar(params).then(function(){
    ...
    <div>
      ...
      <main className="main-panel">
        ...
        <Container fluid={true}>
            <Col style={{top:'-200px'}}>
              <div>
                <img src={user1.avatar} alt="avatar" style={{borderRadius:'50%'}} width="300px" height="300px"/>
              </div>
              <div>
                <input accept="image/*" id="icon-button-file" type="file" hidden={true} onChange={(e)=>{
              ...
              <div>
                <Button color="primary" onClick={()=> setChangePassword(!changePassword)}>Change password</Button>
              </div>
            </Col>   
        <Dialog open={changePassword}>
          ...
          <DialogContent>
            <DialogContentText>
              To change your password, please enter your current password here. And enter new your password then confirm.
            </DialogContentText>
            <TextField
              autoFocus
              margin="dense"
          ...
          <DialogActions>
            <Button color="secondary" onClick={()=> setChangePassword(!changePassword)}>
              Cancel
            </Button>
        ...
        <Dialog open={newAvater}>
          <DialogTitle>
            ...
            <Icon.AiOutlineClose style={{cursor:'pointer', float:'right'}} onClick={()=> {
              setAvatar('');
              setNewAvatar(false);
            }} />
          </DialogTitle>
          <DialogContent>
            <img src={avatar} style={{borderRadius:'50%'}} alt="" width="400px" height="400px"/>
          </DialogContent>
          <DialogActions>
            <Button variant="contained" fullWidth color="primary" onClick={handleNewAvatar}>Set new profile picture</Button>
          </DialogActions>
        </Dialog>
        </Container>
      </main>
